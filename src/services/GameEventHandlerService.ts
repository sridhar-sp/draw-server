...
class GameEventHandlerService {
  ...
  async handleJoin(socket: Socket, errJoiningRoom: Error) {
    ...
    this.gamePlayInfoRepository
      ...
      .then((gamePlayInfo) => {
        ...
        socket
          .to(gameKey)
          .emit(SocketEvents.Room.MEMBER_ADD, SuccessResponse.createSuccessResponse(socket.getUserRecord()));
      })
  ...
  async handleLeave(socket: Socket) {
    try {
      ...
      socket
        .to(socket.getGameKey())
        .emit(SocketEvents.Room.MEMBER_LEFT, SuccessResponse.createSuccessResponse(socket.getUserRecord()));
      ...
      await this.gamePlayInfoRepository
        ...
        .then((gamePlayInfo) => {
          if (gamePlayInfo.isGameFinished() || gamePlayInfo.getDrawingParticipant() == null) {
            const autoSelectWordTaskId =
              gamePlayInfo.autoSelectWordTaskId != null ? gamePlayInfo.autoSelectWordTaskId : "-1";
            const endDrawingSessionTaskId =
              gamePlayInfo.endDrawingSessionTaskId != null ? gamePlayInfo.endDrawingSessionTaskId : "-1";
    ...
    this.gamePlayInfoRepository
      ...
      .then((gamePlayInfo) => {
        ...
        if (leaderBoardData.isGameCompleted()) {
          ...
          if (gamePlayInfo.isCurrentRoundCompleted()) gamePlayInfo.incrementCurrentRound();
          else gamePlayInfo.incrementMatchIndex();

          this.gamePlayInfoRepository.saveGameInfo(gamePlayInfo).then(() => {
            this.scheduleTask(
              TaskType.DISMISS_LEADER_BOARD,
    ...
    this.gamePlayInfoRepository
      ...
      .then((gamePlayInfo) => {
        gamePlayInfo.participants.forEach((participant) => {
          ...
          if (null == drawingParticipant) throw new Error(`No drawing participant found after rotating roles`);
          ...
          if (participant.socketId == drawingParticipant.socketId) {
            response = SuccessResponse.createSuccessResponse(
              GameScreenStatePayload.createSelectDrawingWord(gamePlayInfo.getMaxWordSelectionTimeInSeconds())
            );
          } else {
            response = SuccessResponse.createSuccessResponse(
              GameScreenStatePayload.createWaitForDrawingWord(
                gamePlayInfo.getMaxWordSelectionTimeInSeconds(),
                drawingParticipantSocket.getUserRecord()
              )
            );
          }
      ...
      .then((gamePlayInfo) => {
        ...
        switch (gameScreenState) {
          case GameScreen.State.NONE:
            ...
            if (null == currentDrawingParticipant)
            ...
            if (currentDrawingParticipant.getGameScreenState() == GameScreen.State.SELECT_DRAWING_WORD) {
              return this.createWaitForDrawingWordResponse(
                gamePlayInfo.getMaxWordSelectionTimeInSeconds(),
                currentDrawingParticipant.socketId
              );
            } else {
          ...
          case GameScreen.State.DRAW:
            if (gamePlayInfo.getDrawingWord() == null)
              throw new Error("No drawing word is selected, but the game screen state is set as DRAW");
  ...
  private createWaitForDrawingWordResponse(
    maxWordSelectionTimeInSeconds: number,
    drawingParticipantSocketId: string
  ): SuccessResponse {
    const drawingParticipantSocket = this.socketServer.sockets.sockets[drawingParticipantSocketId];
    if (null == drawingParticipantSocket) throw new Error(`Drawing participant socket instance is not found`);

    return SuccessResponse.createSuccessResponse(
      GameScreenStatePayload.createWaitForDrawingWord(
        maxWordSelectionTimeInSeconds,
        drawingParticipantSocket.getUserRecord()
      )
    );
  }
  ...
  private createViewGameScreenStateResponse(gamePlayInfo: GamePlayInfo) {
    ...
    const drawingParticipantSocket =
      this.socketServer.sockets.sockets[this.getDrawingParticipantSocketIdOrThrow(gamePlayInfo)];
    if (null == drawingParticipantSocket) throw new Error(`Drawing participant socket instance is not found`);

    return SuccessResponse.createSuccessResponse(
      GameScreenStatePayload.createViewStatePayload(
        hint,
        maxDrawingTimeInSeconds,
        drawingParticipantSocket.getUserRecord()
      )
    );
  }
  ...
  async handleSelectWord(fromSocket: Socket, word: string) {
    ...
    this.gamePlayInfoRepository
      ...
      .then((gamePlayInfo) => {
        return this.scheduleDrawingSessionEndTask(gamePlayInfo).then((taskId) => {
      ...
      .then((gamePlayInfo) => {
        gamePlayInfo.participants.forEach((participant) => {
          if (participant.socketId == fromSocket.id) participant.setGameScreenState(GameScreen.State.DRAW);
          else participant.setGameScreenState(GameScreen.State.VIEW);
        });
        return this.gamePlayInfoRepository.saveGameInfo(gamePlayInfo);
      })
      .then((gamePlayInfo) => {
        ...
        if (answer.toLowerCase() == word.toLowerCase()) {
          ...
          if (isEveryoneAnswered) {
            ...
            if (endDrawingSessionTaskId == null)
              throw new Error(
                "Everyone answered but task id to end the current drawing session is missing from record"
              );
  ...
  private scheduleAutoSelectWordTask(gamePlayInfo: GamePlayInfo, word: string, socketId: string): Promise<string> {
    return this.scheduleTask(
      ...
      AutoSelectWordTaskRequest.create(gamePlayInfo.gameKey, socketId, word).toJson(),
      gamePlayInfo.getMaxWordSelectionTimeInSeconds()
    );
  }
  ...
  private scheduleTask(taskType: TaskType, payload: string, taskDelayInSeconds: number): Promise<string> {
    return this.taskScheduler.scheduleTask(
      taskDelayInSeconds * 1000,
      Task.create(taskType, taskDelayInSeconds + 10, payload)
    );
  }
  ...
  private formLeaderBoardPayloadFrom(gamePlayInfo: GamePlayInfo): LeaderBoardData {
    ...
    if (participantsSize == 0) {
      ...
      return LeaderBoardData.create(
        ...
        gamePlayInfo.getTotalRounds(),
        gamePlayInfo.getMatchIndex(),
        true,
        GameEventHandlerService.LEADER_BOARD_VISIBLE_TIME_IN_SECONDS
      );
    }
    ...
    for (let i = 0; i < participantsSize; i++) {
      ...
      if (null != participantSocket) {
        leaderBoardPayload.push(
          UserScore.create(Math.max(participant.getTotalScore(), 0), participantSocket.getUserRecord())
        );
      }
    }
